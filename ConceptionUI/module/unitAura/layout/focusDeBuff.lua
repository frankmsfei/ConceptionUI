...
function D.LOAD.I:LoadFocusDeBuff()
	...
	local function CreateAura(i)
		local a = Button('Conception UI FocusDeBuff '..i, F, 'TOPRIGHT', 'ConceptionUI FocusFrame PowerBar', 'BOTTOMRIGHT', -(cfg.size + cfg.gap) * (i-1) - 10, -28, cfg.size, cfg.size)
			a:SetID(i)
			a:HookScript('OnEnter', function(self) return func.OnEnter(self, 'focus', 'HARMFUL') end)
