...
function D.LOAD.I:LoadPlayerDeBuff()
	...
	local function CreateAura(i)
		local a = Button('Conception UI PlayerDeBuff '..i, F, 'TOPRIGHT', 'ConceptionUI PlayerFrame PowerBar', 'BOTTOMRIGHT', -(cfg.size + cfg.gap) * (i-1) + 25, -66, cfg.size, cfg.size)
			a:SetID(i)
			a:HookScript('OnEnter', function(self) return func.OnEnter(self, 'player', 'HARMFUL') end)
